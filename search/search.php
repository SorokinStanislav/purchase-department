...
<?php 
	if (isset($_GET["search"])) {
		...
		$str = $_GET["query"];
		echo '<h1 style="text-align: center; color:#cc1616">Результаты по запросу: '.$str.'</h1>';		
		...
		$query = "SELECT * FROM products WHERE Number>0 AND (Product='$str' OR Firm='$str') AND NOT EXISTS (SELECT * FROM bin WHERE bin.Login='$login' AND products.Prod_id = bin.Prod_id) ORDER BY products.Product" or die("Ошибка при выполнении запроса.." . mysqli_error($link));
		$result = $link->query($query);
		if($result->num_rows==0){
			echo '<h1 style="text-align:center"> По Вашему запросу ничего не найдено </h1><br>
